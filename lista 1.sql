...
delete from Livros where idioma = 'Françês' and ano_publicacao<1970;

delete from Livros where titulo = 'As Crônicas de Nárnia';

delete from Livros where editora = 'Penguin Book';
...
select avg(quantidade_paginas) from Livros where categoria = 'drama';
select count(quantidade_paginas) from Livros where categoria = 'drama';

select avg(ano_publicacao) from Livors where disponivel = true;

select mim(ano_publicacao) from Lviros;
